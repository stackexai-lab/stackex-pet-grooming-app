...
export type Theme = {
  ...
  typography: {
    bodyFont: string;
    displayFont: string;
    sizes: Record<'caption' | 'body' | 'bodyLarge' | 'title' | 'display', string> & Partial<Record<'card' | 'label' | 'button', string>>;
    weights: Record<'regular' | 'medium' | 'bold', number> & { extraBold?: number };
...
const premium: Variation = {
  ...
  typography: {
    bodyFont: 'Manrope, "Noto Sans Arabic", ui-sans-serif, system-ui, sans-serif',
    displayFont: 'Sora, "Noto Sans Arabic", ui-sans-serif, system-ui, sans-serif',
    sizes: { caption: '0.75rem', body: '0.875rem', bodyLarge: '0.9375rem', title: '1.625rem', display: '1.75rem', card: '1.125rem', label: '0.6875rem', button: '0.90625rem' },
    weights: { regular: 400, medium: 500, bold: 700, extraBold: 800 }, headingWeight: 700, buttonWeight: 800,
  },
